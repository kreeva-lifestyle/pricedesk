...
- `myntra-price` — server-side scrape of live Myntra selling prices by Style ID (max 10/call, per-IP rate limit in `pd_myntra_rate_limits`); parsing strategies (JSON-LD → embedded SPA state deep-walk `__myx`/`__INITIAL_STATE__`/`pdpData` for `discounted`/`sellingPrice`/… → regex → `og:price` meta → `pdp-price` DOM, plus gateway JSON API fallback) live in pure `parse.ts` (Vitest-covered). **Myntra bot-blocks datacenter IPs** (serves a 483-byte "Site Maintenance" stub to Supabase, real page to residential browsers) — so live use requires a residential-IP scraping API: set env secrets `SCRAPER_TEMPLATE` (provider-agnostic template with `{url}`/`{key}` placeholders, e.g. `https://api.scraperapi.com/?api_key={key}&url={url}&country_code=in`) and `SCRAPER_KEY`; `buildScraperUrl` (pure, in `parse.ts`) wraps the target URL and `isMyntraHost`/timeout relax in scraper mode. Unset ⇒ direct fetch (blocked by Myntra in production, fine for tests). **Free alternative to the paid scraper:** `tools/myntra-fetch/` is a standalone Node CLI the user runs on their own laptop (residential IP Myntra trusts) — it reads active Myntra SKUs from `pd_skus_v2`/`pd_brands`, fetches each PDP, parses with `parse.mjs` (a plain-JS mirror of `parse.ts`, drift-guarded by `tests/edge-functions/myntra-fetch-sync.test.js`), and writes `pd_live_prices` + pings `pd_sync` exactly like the app — so the "Myntra ₹" column updates live with no Edge Function or third-party service involved. On a parse miss the error carries a page fingerprint (`fingerprintSummary`: blocked-vs-new-markup, byte size, markers present). Request `{debug:true}` returns a capped page excerpt per style; browser console helper `await myntraDebug('<styleId>')` surfaces it for tuning the parser to live markup. Client stores results in `app_data` key `pd_live_prices` (`LIVE_PRICES` global, `{[styleId]:{price,mrp,ts,err}}`) shown in the SKU table's "Myntra ₹" column beside Cust. Paid — **display-only, never feeds `calcSKU`** (keep `live_prices` OUT of `CALC_INPUT_TYPES`)
