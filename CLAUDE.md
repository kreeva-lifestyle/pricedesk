...
- The app is **display + status only**: `pd_live_prices` (`LIVE_PRICES` global, `{[styleId]:{price,mrp,ts,err}}`) → "Myntra ₹" column beside Cust. Paid (green match / red-amber drift / ✕ error / — none) + mobile card cell. **Display-only, never feeds `calcSKU`** (keep `live_prices` OUT of `CALC_INPUT_TYPES`). No in-app fetch button — the laptop is the only fetcher.
